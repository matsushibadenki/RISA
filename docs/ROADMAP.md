...
- [Next] specialization・mergeの自動提案規則を実装し、親候補を超える追加価値を独立development/finalで比較 / Implement automatic specialization and merge proposal rules and compare added value over parents on independent development and final evidence / 实现自动分化与合并提案规则，并在独立development/final证据上比较其相对父候选的附加价值
