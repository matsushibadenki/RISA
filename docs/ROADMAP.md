...
- [Next] G3.2全phase帰属: 事前更新予測を各Eventで介入比較し、未知actor/targetの独立probeも追加して、構造が必要なqueryと具体記録で足りるqueryを分離 / Compare component interventions at every pre-update Event and on independent unseen-actor/target probes, separating queries that require transfer from those satisfied by grounded records / 在每个Event更新前及独立的未见actor/target探针上比较组件干预，区分需要结构转移与仅靠具体记录即可回答的query
...
追加の開発診断: [オンライン採用 / Online adoption / 在线采纳](G3.2-Online-Validation-Development-2026-09-18.md)、[Primitive分裂 / Primitive split / 原语分裂](G3.2-Split-Drift-Opportunity-2026-09-18.md)、[文脈別drift / Contextual drift / 按情境漂移](G3.2-Contextual-Drift-2026-09-18.md)、[条件付き分裂提案 / Conditional split proposal / 条件式分裂提案](G3.2-Contextual-Split-Proposal-2026-09-18.md)。機構が作動しても回復効果がない場合と、提案消失・局所誤差の希釈により機会がない場合を別々に記録する。
