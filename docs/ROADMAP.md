...
- [Next] G3.2構造寄与の独立対照: 同じprobeでrole条件付き具体遷移baselineと比較し、構造介入が未知queryを有利に変えるworldを開発用に探す。保持・回復・構造数・Replay費用・誤一般化を同時に測る / Compare against a role-conditioned grounded transition baseline on identical probes and develop a world where structural intervention improves unseen queries; jointly measure retention, recovery, structure count, Replay and false generalization / 在相同探针上与按role条件化的具体迁移基线比较，并开发结构干预能改善未见查询的环境；同时测量保留、恢复、结构数量、Replay成本与错误泛化
...
追加の開発診断: [オンライン採用 / Online adoption / 在线采纳](G3.2-Online-Validation-Development-2026-09-18.md)、[Primitive分裂 / Primitive split / 原语分裂](G3.2-Split-Drift-Opportunity-2026-09-18.md)、[文脈別drift / Contextual drift / 按情境漂移](G3.2-Contextual-Drift-2026-09-18.md)、[条件付き分裂提案 / Conditional split proposal / 条件式分裂提案](G3.2-Contextual-Split-Proposal-2026-09-18.md)、[全phase予測寄与 / Phase-wide readout attribution / 全阶段预测贡献](G3.2-Readout-Attribution-2026-09-18.md)。機構が作動しても回復効果がない場合と、提案消失・局所誤差の希釈により機会がない場合を別々に記録する。
